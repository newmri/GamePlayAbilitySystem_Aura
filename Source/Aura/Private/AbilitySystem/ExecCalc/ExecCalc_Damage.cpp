	...
	for (auto d : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -0.5f)
		{
			...
			if (bDebuff)
			{
				...
				const auto DebuffDamage = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Damage, false, -1);
				const auto DebuffDuration = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Duration, false, -1);
				const auto DebuffFrequency = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Frequency, false, -1);
...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const auto& d : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamageHit(EffectContextHandle))
		{
